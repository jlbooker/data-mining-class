...
class TestMethods(unittest.TestCase):
    ...
    def make_statistics_tests(self):
        ...
        for name, rng, unlock in [('median', randi, ['numpy.median']),
                                  ('nanmean', nanrandi,
                                   ['numpy.%s' % s for s in ['nanmean', 'nanmedian', 'nanvar', 'nanstd']])]:
            path_correct = 'numpy.%s' % name
            test = {
                'unlock': unlock,
                'import': ['math'],
        ...
        for name, rng, unlock in [('percentile', randi, ['numpy.%s' % s for s in ['percentile', 'nanpercentile']])]:
            path_correct = 'numpy.%s' % name
            test = {
                'unlock': unlock,
                'import': ['math'],
        ...
        tests.append({
            'unlock': [path_correct, 'matplotlib.pyplot.hist2d'],
            'import': ['math'],
            'params': [[{
                'func': path_correct,
    ...
    def make_distance_tests(self):
        ...
        unlock = [path_correct, 'scipy.spatial.distance.pdist', 'scipy.spatial.distance.squareform']
        tests.append({
            'unlock': unlock,
            'import': ['math'],
            'params': [[{
        ...
        with open(file_path) as fp:
            for line in fp.read().splitlines():
                if re.match(r"^\s*(import|from)", line):
                    legal_import = True
                    import_as = r"^\s*import\s+([\w\.]+)\s+as\s+([\w\.]+)$"
                    imports = r"^\s*import\s+([\w\.,\s]+)$"
                    from_import = r"^\s*from\s+([\w\.]+)\s+import\s+(.+)$"
                    if re.match(import_as, line):
                        m = re.match(import_as, line)
                        pkg = m.group(1)
                        if pkg not in allowed_imports:
                            legal_import = False
                    elif re.match(imports, line):
                        m = re.match(imports, line)
                        packages = re.split(r"\s*,\s*", m.group(1))
                        for pkg in packages:
                            if pkg not in allowed_imports:
                                legal_import = False
                                break
                    elif re.match(from_import, line):
                        m = re.match(from_import, line)
                        base_package = m.group(1)
                        functions = re.split(r"\s*,\s*", m.group(2))
                        packages = ['%s.%s' % (base_package, function) for function in functions]
                        for pkg in packages:
                            if pkg not in allowed_imports:
                                legal_import = False
                                break
                    ...
                    #     if re.match("\s*from %s import %s$" % ('.'.join(terms[:-1]), terms[-1]), line) or re.match("\s*from %s import %s$" % ('.'.join(terms[:-1]), terms[-1]), line) \
    ...
    def check_csv(self, files):
        import requests
        import pandas as pd
        from io import StringIO
        for csv_student in files:
            with self.subTest(test=csv_student):
                url_correct = 'http://www.cs.appstate.edu/~rmp/cs5710/tests/%s' % csv_student
                df_correct = pd.read_csv(StringIO(requests.get(url_correct).content.decode('utf-8')))
                df_student = pd.read_csv(csv_student)
                c_correct = df_correct.columns
                c_student = df_student.columns
                msg = 'column labels do not match in %s: check %s' % (csv_student, url_correct)
                np.testing.assert_array_equal(c_student, c_correct, err_msg=msg)
                for c in c_correct:
                    na_student = df_student[c].isnull()
                    na_correct = df_correct[c].isnull()
                    msg = 'missing values in column "%s" do not match %s: check %s' % (c, csv_student, url_correct)
                    np.testing.assert_array_equal(na_student, na_correct, err_msg=msg)
                    val_student = df_student.loc[~na_correct, c]
                    val_correct = df_correct.loc[~na_correct, c]
                    msg = 'values in column "%s" do not match %s: check %s' % (c, csv_student, url_correct)
                    np.testing.assert_array_equal(val_student, val_correct, err_msg=msg)
                print('%s passes!' % csv_student)

    def check_pca(self, allowed_imports):
        student = 'cs5710.apps.pca.main'
        f_student = get_function(student)
        images = ['files/pca.png']
        with self.subTest(test='pca'):
            self.check_imports(f_student, allowed_imports)
            f_student()
            self.check_images(images)


    def check_data_processing(self, allowed_imports):
        student = 'cs5710.apps.data_processing.main'
        f_student = get_function(student)
        files = ['files/cleaned.csv', 'files/joined.csv']
        with self.subTest(test='data_processing'):
            self.check_imports(f_student, allowed_imports)
            f_student()
            self.check_csv(files)
    ...
    def check_visualization(self, allowed_imports):
        ...
        with self.subTest(test='visualization'):
            self.check_imports(f_student, allowed_imports)
            f_student()
            images = ['files/Fig3.%d.png' % n for n in [9, 11, 13, 15, 17, 19]]
            images += ['files/Fig3.12-%d.png' % n for n in range(3)]
            images += ['files/Fig3.14-%d.png' % n for n in range(4)]
            images += ['files/Fig3.16-%d%d.png' % (i, j) for i in range(4) for j in range(4) if i != j]
            self.check_images(images)
    ...
    def check_self_similarity(self, allowed_imports):
        ...
        with self.subTest(test='self_similarity'):
            self.check_imports(f_student, allowed_imports)
            f_student()
            images = ['files/%s.png' % t for t in ['timbre', 'pitch']]
            self.check_images(images)
    ...
    def check_overfitting(self, allowed_imports):
        ...
        with self.subTest(test='overfitting'):
            ...
            data_file = 'Fig4.23'
            f_student(data_file)
            images = ['files/Fig4.22.png'] + ['files/Fig4.23_%s.png' % t for t in ['tree', 'knn']]
            self.check_images(images)
    ...
    def check_bayes_error(self, allowed_imports):
        ...
        with self.subTest(test='bayes_error.UniformGMMClassifier.__init__'):
            ...
            self.assertIsNone(clf.gmm.thresh, 'clf.gmm.thresh should be None')
        ...
        with self.subTest(test='bayes_error.UniformGMMClassifier.main'):
            ...
            self.check_images(['files/bayes_error.png'])
    ...
    def test_all(self):
        ...
        # set the program level you want to test.
        program = 5
        all_apps = False
        ...
        # Program 2
        if all_apps or program == 2:
            self.check_data_processing(['pandas', 'math', 'string'])

        # Program 3
        if program >= 3:
            unlocked_imports += self.check_against_standard(unlocked_imports, self.make_distance_tests())
        if all_apps or program == 3:
            self.check_self_similarity(['json', 'matplotlib.pyplot', 'numpy.exp'] + unlocked_imports)
        ...
        # Program 4
        if all_apps or program == 4:
            self.check_pca(unlocked_imports + ['matplotlib.pyplot', 'sklearn.datasets.load_iris'])

        # Program 5
        if all_apps or program == 5:
            legal_imports = ['matplotlib.pyplot.%s' % s for s in
                             ['clf', 'gray', 'colorbar', 'xlabel', 'ylabel', 'title', 'savefig', 'boxplot', 'pie',
                              'axis', 'plot', 'legend', 'scatter', 'gca', 'clabel', 'contour', 'figure', 'subplot']] + \
                            ['mpl_toolkits.mplot3d.Axes3D', 'scipy.misc.imread', 'numpy.arange', 'numpy.linspace',
                             'sklearn.datasets.load_iris']
            self.check_visualization(legal_imports + unlocked_imports)
        ...
        if all_apps or program == 6:
            self.check_overfitting(['matplotlib.pyplot', 'numpy'] + unlocked_imports)

        # Program 7
        if program >= 7:
            unlocked_imports += self.check_against_standard(unlocked_imports, self.make_cluster_tests())
        if all_apps or program == 7:
            self.check_bayes_error(['matplotlib.pyplot', 'numpy'] + unlocked_imports)
